    ...
    return {
      ...
      lint: (activeEditor) => {
        ...
        if(/^C\+\+/.test(grammar)) {
          //const language = "c++";
          args.push("-xc++");
          args.push(...atom.config.get("linter-clang.defaultCppFlags").split(/\s+/));
        }
        if(grammar === "Objective-C++") {
          //const language = "objective-c++";
          args.push("-xobjective-c++");
          args.push(...atom.config.get("linter-clang.defaultObjCppFlags").split(/\s+/));
        }
        if(grammar === "C") {
          //const language = "c";
          args.push("-xc");
          args.push(...atom.config.get("linter-clang.defaultCFlags").split(/\s+/));
        }
        if(grammar === "Objective-C") {
          //const language = "objective-c";
          args.push("-xobjective-c");
          args.push(...atom.config.get("linter-clang.defaultObjCFlags").split(/\s+/));
        }

        args.push(`-ferror-limit=${atom.config.get("linter-clang.errorLimit")}`);
        if(atom.config.get("linter-clang.suppressWarnings")) {
          args.push("-w");
        }
        ...
        atom.config.get("linter-clang.includePaths").forEach((path) =>
          args.push(`-I${path}`)
        );
